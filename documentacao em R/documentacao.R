...
dados_formatados <- cyclist2022v4 %>%
  mutate(KM_percorridos = format(KM_percorridos, nsmall = 2),
         tempo_total_min = format(tempo_total_min, nsmall = 2))

dados_formatados$KM_percorridos <- as.numeric(dados_formatados$KM_percorridos)

dados_formatados$tempo_total_min <- as.numeric(dados_formatados$tempo_total_min)

# Dividir a coluna "KM_percorridos" por 1.000.000 e criar uma nova coluna chamada "KM_percorridos_divididos"
dados_formatados$KM_percorridos_divididos <- dados_formatados$KM_percorridos / 1000000

# Dividir a coluna "tempo_total_min" por 1.000.000 e criar uma nova coluna chamada "tempo_total_min_divididos"
dados_formatados$tempo_total_min_2 <- dados_formatados$tempo_total_min_1 / 100

head

head(dados_formatados)
#####

cyclist2022v4 <- read.csv("C:/Users/welin/Desktop/GOOGLE/Estudos de caso/1/cyclist2022v4.csv")

write_xlsx(cyclist2022v4, "C:/Users/welin/Desktop/GOOGLE/Estudos de caso/1/cyclist2022v4.xlsx")


head(cyclist2022v4)

cyclist2022v4$data <- as.Date(cyclist2022v4$data)

# Agrupar por data e somar o tempo total
data_min <- cyclist2022v4 %>%
  group_by(data, member_casual) %>%
  summarise(tempo_total = sum(tempo_total_min))
######

# Agrupar por data e somar os quilometros percorridos
data_km <- cyclist2022v4 %>%
  group_by(data, member_casual) %>%
  summarise(km_total = sum(KM_percorridos))


# Contar o número de ocorrências por estacao do ano
data_estacao <- cyclist2022v4 %>%
  count(data, estacao, member_casual)

numero_de_linhas33 <- nrow(data_dia_semana)
print(numero_de_linhas33)


# Contar o número de ocorrências por hora *mais linhas q as outras*
data_hora <- cyclist2022v4 %>%
  count(data, hora, member_casual)

# Contar o número de ocorrências por mes
data_mes <- cyclist2022v4 %>%
  count(data, mes, member_casual)

# Contar o número de ocorrências por dias da semana

data_dia_semana <- cyclist2022v4 %>%
  count(data, dia_semana, member_casual)


# Unir todos os datasets 

novo_dataset <- cbind(data_min, data_km, data_estacao, data_mes, data_dia_semana)
print(novo_dataset$data_dia_semana)
view(novo_dataset)


novo_dataset2 <- cbind(data_min, data_km, data_estacao, data_mes, data_dia_semana, data_hora)
head(novo_dataset)
view(novo_dataset2)


novo_dataset$km_total <- format(novo_dataset$km_total, big.mark = ".", decimal.mark = ",")

novo_dataset$tempo_total <- format(novo_dataset$km_total, big.mark = ".", decimal.mark = ",")

write.csv(dados_formatados, "C:/Users/welin/Desktop/GOOGLE/Estudos de caso/1/dados_formatados.csv", row.names = FALSE)

write.csv(data_hora, "C:/Users/welin/Desktop/GOOGLE/Estudos de caso/1/data_hora.csv", row.names = FALSE)



C:/Users/welin/Desktop/GOOGLE/Estudos de caso/1/
